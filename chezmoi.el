...
;; BUG: returns t for chezmoi scripts which are not managed files.
(defun chezmoi-source-file-p (file)
  "Return non-nil if FILE is a chezmoi source file."
  (setq file (expand-file-name file))
  (and (string-match-p (regexp-quote (chezmoi-source-path))
                       file)
       (not (string-match-p (rx (eval (chezmoi-source-path))
                                (or ".chezmoidata/"
                                    ".chezmoiexternals/"
                                    ".chezmoiscripts/"
                                    ".chezmoitemplates/"))
...
the buffer editing the source state is saved the target state is kept in sync."
  (interactive
   (list (chezmoi--completing-read "chezmoi edit: "
			           (chezmoi-managed-files)
			           'project-file)))
...
Traget file is in A buffer, source file in B buffer."
  (interactive
   (list (chezmoi--completing-read "chezmoi merge: "
