...
If FILE is a source file — run \"chezmoi apply\" to override target file,
for target file — copy it to the source file."
  (interactive (list (buffer-file-name) current-prefix-arg))
  (or file (setq file (buffer-file-name)))
  (let (other-file)
    (if (zerop (if (chezmoi-target-file-p file)
                   ;; target file
                   (progn
                     (setq other-file (chezmoi-source-file file))
                     (call-process "chezmoi" nil nil nil
                                   "re-add" file
                                   (if arg " --force" "")))
                 ;; else source state
